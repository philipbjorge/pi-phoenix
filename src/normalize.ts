  ...
  for (const block of content) {
    ...
    if (b.type === "toolCall") {
      calls.push({
        ...
        function: {
          name: b.name,
          arguments: typeof b.arguments === "string" ? b.arguments : safeStringify(b.arguments),
        },
      });
    }
  }
...
export function normalizeMessages(
  messages: unknown[],
  maxBytes: number,
): Array<{ role: string; content: string; toolCalls?: NormalizedToolCall[]; toolCallId?: string }> {
  const result: Array<{ role: string; content: string; toolCalls?: NormalizedToolCall[]; toolCallId?: string }> = [];
  ...
  for (const msg of messages) {
    ...
    const text = captureValue(extractText(m.content), maxBytes);
    const entry: { role: string; content: string; toolCalls?: NormalizedToolCall[]; toolCallId?: string } = {
...
export function normalizeToolInput(toolName: string, input: unknown, maxBytes: number): string {
  ...
  if (toolName === "write" && typeof value.path === "string") {
    const content = typeof value.content === "string" ? value.content : safeStringify(value.content);
  ...
  if (toolName === "edit" && typeof value.path === "string") {
    ...
    const parts = [value.path, `${edits.length} edit block${edits.length === 1 ? "" : "s"}`];
    const first = edits[0];
...
export function normalizeToolOutput(result: unknown, maxBytes: number): string {
  ...
  const value = result as {
    content?: unknown;
    details?: { path?: unknown; edits?: unknown; targetFile?: unknown } | unknown;
  };
  ...
  if (details && typeof details === "object") {
    const obj = details as { path?: unknown; edits?: unknown; targetFile?: unknown };
    const path = typeof obj.path === "string" ? obj.path : typeof obj.targetFile === "string" ? obj.targetFile : undefined;
    if (path && Array.isArray(obj.edits)) {
      return captureValue(`applied ${obj.edits.length} edits to ${path}`, maxBytes);
    }
    if (path) {
